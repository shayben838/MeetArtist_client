...
function Header({ user, logOut, dataAPI }) {
    ...
    return (
        <div className={"wrap_header"}>
            <nav style={{ backgroundColor: style.mainColor }} className="navbar navbar-expand-lg  fixed-top navbar-dark   wrap_header">
                <Link to="/">
                    <span className="meet_artist_logo navbar-brand " href="#">MeetArtist</span>
                </Link>
                    ...
                    <ul className="navbar-nav mr-auto">
                        {!user &&
                            <Link to="/log_in" className="headers_link">
                                <li className="nav-item " >
                                    <span className="  nav-link" href="#">Sign In</span>
                                </li>
                            </Link>
                        }
                        ...
                        {!user &&
                            <Link to="/SignUp" className="headers_link">
                                <li className="nav-item">
                                    <span className="nav-link" href="#">Sign Up</span>
                                </li>
                            </Link>
                        }
                        ...
                        <div className="d-none d-lg-block  wrap_search_hedear">
                            <div className="wrap_ul">
                                <div className="wrap_input">
                                    <input id={"input_search_headear"} className="input_drop_down" placeholder="Search" name={buttonName} type="text" onChange={onInputChange} autoComplete="off" />
                                </div>
                                {lenAuto > 0 &&
                                    <div className="wrap_auto_headear pb-2">
                                        {autoComplite.map((item, key) =>
                                            <button onClick={() => onClicFilter({ name: 'display_name', value: { id: item.id, name: item[option] } })} className="button_drop_down d-block" id={item.id} value={item[option]} key={key}><i className="fas fa-search"></i> {item[option]} </button>
                                        )}
                                    </div>
                                }
                            </div>
